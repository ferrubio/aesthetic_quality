...
def CaffeNet(data, label=None, train=True, num_classes=2, fc_nodes=4096, learn_all=False):
    ...
    n.fc6, n.relu6 = fc_relu(n.pool5, fc_nodes, param=learned_param)
    if train:
        n.drop6 = fc7input = L.Dropout(n.relu6, in_place=True)
    else:
        fc7input = n.relu6
        
    n.fc7, n.relu7 = fc_relu(fc7input, fc_nodes, param=learned_param)
    if train:
    ...
    # change fc values names for errors trying to load the weights
    if fc_nodes != 4096:
        n.__setattr__('fc6_aes', n.fc6)
        n.__setattr__('fc7_aes', n.fc7)
    if num_classes != 1000:
        n.__setattr__('fc8_aesthetic', fc8)
...
def AesNet_CaffeNet(train=True, learn_all=False, subset=None, source_path='', fc_nodes=4096):
    if subset is None:
    ...
    return CaffeNet(data=data, label=label, train=train, num_classes=2, fc_nodes=fc_nodes, learn_all=learn_all)
    
    

def VGG16(data, label=None, train=True, num_classes=1000, fc_nodes=4096, learn_all=False):
    ...
    # change fc values names for errors trying to load the weights
    if fc_nodes != 4096:
        n.__setattr__('fc6_mod', n.fc6)
        n.__setattr__('fc7_mod', n.fc7)
    if num_classes != 1000:
        n.__setattr__('fc8_mod', fc8)
...
def AesNet_VGG16(train=True, learn_all=False, subset=None, source_path='', fc_nodes=4096):
    if subset is None:
        subset = 'train' if train else 'test'
    ...
    if subset == 'train':
        ...
        data = L.Input(input_param=dict(shape=dict(dim=[100, 3, 224, 224])))
        label = None
        train = False
        learn_all=False
        
    return VGG16(data=data, label=label, train=train, num_classes=2, fc_nodes=fc_nodes, learn_all=learn_all)
